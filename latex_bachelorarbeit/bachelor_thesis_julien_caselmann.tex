...
%TODO: structure of the model diagram gut geworden?
...
\begin{figure}%TODO: evtl in ein Appendix?
	\centering %(or not)
	\def\svgwidth{75pt}
...
%TODO: proof of transition rates? find better wording
...
%TODO: yet again, proof is not the correct term
\begin{proof}
...
\begin{prop}
	...
	\end{align}
\end{prop}
\begin{proof}\phantom{lol}\\%TODO: die zwei fetten 1. und 2. gliederungen müssen nicht sein
...
\begin{prop}[Theoretical stationary point of the alternate Model]\label{prop:theor_stat_point_my_model}\phantom{lel}\newline
	...
	For $n_X = \max \lbrace a\left(1-2i^*\right), 0\rbrace$, $n_Y = a\left(2i^* - 1\right) + n_X$ and $\theta_X = \theta_Y = \frac{1-2n}{1+2n}$ with\newline $n \coloneqq ai^* + n_X$, there always is a stationary point at $x^* = \frac{1}{2}$.%TODO: undergoes bifus at... ? dafür bräuchte ich taylor... oder wissen wir, dass es denselben bums amselben theta macht? (wie müllers) 
\end{prop} 
\begin{proof}
	...
	So we have a stationary point at $\left(\frac{\alpha + b}{\beta}, \frac{b}{2\left(\alpha + b\right)} - \frac{b}{\beta}, \frac{1}{2}\right)$. Now we have to adjust the parameters occurring in \eqref{prop:det_limit_my_model} such that $\dot{x} = 0$ holds. To do so, we will compare the two parts of $\dot{x}$ and make sure they are the same, which will make them cancel each other out and yield the wanted result. We immediately see that $\mu x = \mu\left(1-x\right)$ for $x = \frac{1}{2}$. We may now write down two equations that have to hold for equality of both sides:%TODO: weird phrasing
...
Furthermore, when we take a look at the system as whole, as in Figure \ref{fig:time_behaviour_my_model}, and let the simulation begin at $s_0 = 0.8, i_0 = 0.2, x_0 = 0.2$ (in its entirety?)%TODO
, we see crystal clear oscillations of $s$ and $x$ around $0.5$ and of $i$ around $0.06$.
...
Additionally, we have this diagram Mueller wanted me to do with the Eigenvalues, which shows the existence of a Hopf point %TODO: diagramm zaubern 

%TODO: alle oberen todos (aus bifi-anal) abarbeiten: dann fehlt nur noch saddle-node, danach ist bifi-analyse beednet
...
\begin{figure}%TODO: do i need to use it? and can i? (is BA private use?)
	\centering
