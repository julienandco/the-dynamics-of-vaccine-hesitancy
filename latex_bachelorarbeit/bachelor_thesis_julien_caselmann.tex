...
%TODO: tum citation guide: figures und tables wo kommt caption hin?
...
To get a better understanding of the dynamics that are hidden in those equations, we propose to take a look at Figure \ref{fig:my_model}:
...
Furthermore, there are multiple indicators that imply the existence of a Hopf bifurcation in the system. Writing down a general stationary point analytically for our model turns out to be very unpractical, but we are able to ``construct" one at $x=0.5$:
\begin{prop}[Theoretical stationary point of the alternate Model]\label{prop:theor_stat_point_my_model}\phantom{lel}\newline
	...
	For $n_X = \max \lbrace a\left(1-2i^*\right), 0\rbrace$, $n_Y = a\left(2i^* - 1\right) + n_X$ and $\theta_X = \theta_Y = \frac{1-2n}{1+2n}$ with\newline $n \coloneqq ai^* + n_X$, $x^* = 0.5$ always is a stationary point.%TODO: undergoes bifus at... ? 
\end{prop} 
\begin{proof}
	We set $x^* = 0.5$. It is now very easy to write down $s^*$ and $i^*$:
	\begin{align}
	\begin{split}\label{eq:s_in_theo_stat_point}
		0 &\overset{!}{=} \dot{s^*} \overset{\eqref{model:my_model}}{=}-\beta s^*i^* + 0.5b - bs^*\\
	...
	So we have a stationary point at $\left(\frac{\alpha + b}{\beta}, \frac{b}{2\left(\alpha + b\right)} - \frac{b}{\beta}, 0.5\right)$. Now we have to adjust the parameters occurring in \eqref{prop:det_limit_my_model} such that $\dot{x} = 0$ holds. To do so, we will compare the two parts of $\dot{x}$ and make sure they are the same, which will make them cancel each other out and yield the wanted result. We immediately see that $\mu x = \mu\left(1-x\right)$ for $x = 0.5$. We may now write down two equations that have to hold for equality of both sides:
	
	\begin{align}
		\begin{split}\label{eq:numerator_equality}
		\theta_X\left(0.5+n_x+ai\right) &= \theta_Y\left(0.5+n_Y+a(1-i)\right)\\
		&\end{split}\\
		\begin{split}\label{eq:denominator_equality}
		\theta_X\left(0.5+n_X+ai\right) + 0.5+n_Y+a(1-i) &= 0.5+n_X+ai+\theta_Y\left(0.5+n_Y+a(1-i)\right)\end{split}
	\end{align}
	
	One quickly notices that both these equations hold true for $n_X = n_Y = n, \theta_X = \theta_Y = \theta$ and $i = 0.5$, but we do not want to fix $i$. So we have to look at another constellation of parameters, that ensures the equality of \eqref{eq:numerator_equality} and \eqref{eq:denominator_equality}. We find the following two possibilities:
...
We may now use the insights about the theoretical stationary point $\left(s^*, i^*, 0.5\right)$ from \eqref{prop:theor_stat_point_my_model} and set the parameters of our model as follows: 
...
Additionally, we have this diagram Muelle rwanted me to do with the Eigenvalues, which shows the existence of a Hopf point %TODO: diagramm zaubern 
