...
\begin{align*}
	\rho &= P_t\left(m-1\right)\frac{1-p}{N\left(N+N_X+N_Y-1\right)}\left(m+N_X-1\right)\left(N-m+1\right)\\
	...
	&\hookrightarrow \mu\left(N-X_t\right)\frac{X_t + N_X}{N + N_X + N_Y}
\end{align*}
...
As also shown by M\"uller and Tellier, the pitchfork bifurcation changes into a saddle-node bifurcation for slightly differing numbers of zealots $n_X$ and $n_Y$. %TODO: elaborate, mueller notizen aus ipad fragen, meiner meinung nach geht bei x=0,5 nichts... vllt woanders
...
\begin{prop}[Theoretical stationary point of the alternate Model]\label{prop:theor_stat_point_my_model}\phantom{lel}\newline
	...
	For $n_X = \max \lbrace a\left(1-2i^*\right), 0\rbrace$, $n_Y = a\left(2i^* - 1\right) + n_X$ and $\theta_X = \theta_Y = \frac{1-2n}{1+2n}$ with\newline $n \coloneqq ai^* + n_X$, there always is a stationary point at $x^* = \frac{1}{2}$.%TODO: undergoes bifus at... ? dafür bräuchte ich taylor... oder wissen wir, dass es denselben bums amselben theta macht? (wie müllers) s. ein todo drüber, meiner meinung nach undergoed der gar nichts 
\end{prop} 
\begin{proof}
