...
%TODO papa: exemplar der arbeit an rki schicken???
...
The R Scripts that generated Figures \ref{fig:alu_vs_x}, \ref{fig:time_behaviour_my_model}, \ref{fig:eigenvalue_diagram_hopf} and \ref{fig:data_fits} can be found in the Appendix.
...
\begin{figure}[h!]%TODO second: axis labels are very small compared to figure above. Maybe put them on different pages?
	\centering
...
We computed the deterministic limit of Model \ref{model:my_model} in Subsection \ref{subsec:det_lim_comp} so that we could analyse its behaviour over time and visualise it quite nicely. We now want to fit the model onto real-life data and see how well it performs. It has been shown in \cite{Etheridge2011, Hosel2020}, that weak effect models are better suited for this goal, especially considering realism, which is why the next step will be to compute the weak effect limit of our model.
...
Of course we do not claim our model to be exhaustive and as is well-known: ``no model is right". All of the comments, assumptions and claims made in the following base on the results our work yielded. We did not take a look at additional cofactors, such as educational or social background or financial status. These are all possible influences on the behaviour, just as communication and filter bubbles and represent interesting research topics for further work.

%TODO fifth: evtl noch mehr zu den Bildern

\section{Discussion} %TODO now: noch in verbindung mit anderen arbeiten bringen (hab jz zwei)
...
\input{appendix_bachelor_thesis_julien_caselmann.tex}
