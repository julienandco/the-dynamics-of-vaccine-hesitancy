...
%TODO last: many weird sources, fix them. all last names and titles pls NOT capslock. all titles in same font (italic or normal, but not mixed). get maxima citation from maxima FAQ page. adjust KBV overfull link. 

%TODO: cite xppaut just as maxima

%TODO last: sort sources numerically in every multi-cite bracket
%TODO: welche Autoren bei MÜllers paper?
...
The R script that generated Figures \ref{fig:alu_vs_x}, \ref{fig:time_behaviour_my_model} and \ref{fig:eigenvalue_diagram_hopf} can be found in the Appendix.
...
\begin{prop}[\textbf{A Stationary Point of the Alternate Model}]\label{prop:theor_stat_point_my_model}\phantom{lel}\newline
...
\begin{figure}[h!]%TODO: axis labels are very small compared to figure above. Maybe put them on different pages?
	\centering
...
We computed the deterministic limit of Model \ref{model:my_model} in Subsection \ref{subsec:det_lim_comp} so that we could analyse its behaviour over time and visualise it quite nicely. We now want to fit the model to real-life data and see how well it fits. It has been shown in \cite{Etheridge2011, Hosel2020}, that weak effect models are better suited for this goal, especially considering realism, which is why the next step will be to compute the weak effect limit of our model.

%TODO: enough reformulated this intro? since it is copy-paste from muellers paper
...
%TODO: what is this z_t supposed to mean?
\begin{thm}[\textbf{Weak Effect Limit of the Alternate Model}]\label{theorem:weak_eff_lim}
...
. This insight shows evidence of oscillations in the system, as can be seen in Figure \eqref{fig:time_behaviour_my_model}. It would be very interesting for future work to investigate the existence of a Takens-Bogdanov bifurcation in the presented system.\newline
 ...
 %TODO: iwas von wegen: das zeigt mein modell ist halbwegs realistisch
 %TODO: zusammenfassugn meines modells: ich habe also das und das mehr erreicht/nicht geschafft als bauch.. bla bla
