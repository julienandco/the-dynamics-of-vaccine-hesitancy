...
%TODO: bifurcations erklären! evtl auch noch normalform z.B.
...
As we know from \cite{Hale1991}, two purely imaginary eigenvalues crossing the imaginary axis imply the existence of a Hopf. This fact can be seen in Figure \ref{fig:eigenvalue_diagram_hopf}, which depicts the spectrum of the linearisation $J$ for variations of $\mu$.
%TODO: nächsten satz schöner schreiben
(remember: linearisation is 2-d, so can only have 2 eigenvalues and as we know: p(z) = 0 => p(z-conj) = 0, so every pair of complex numbers represents the two eigenvalues for one value of $\mu$).
