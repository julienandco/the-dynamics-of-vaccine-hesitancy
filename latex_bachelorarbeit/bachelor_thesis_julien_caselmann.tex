...
\begin{document}
%set to true if you want the version with appendix, false otherwise
\setboolean{appendix}{true}

%set to true if you want the signed version
\setboolean{signed}{false}
...
\ifthenelse{\boolean{appendix}}{The R Scripts that generated Figures \ref{fig:alu_vs_x}, \ref{fig:time_behaviour_my_model}, \ref{fig:eigenvalue_diagram_hopf} and \ref{fig:data_fits} can be found in Appendix \ref{app:first} and \ref{app:second}, respectively.}{}
...
We may now easily reinterpret this model to depict the opinion of a population. If we consider $X_t$ and $Y_t$ as the number of supporters of opinion $X$ and $Y$ respectively and replace ``death" by ``not sharing this opinion any more" and ``birth" by ``adopting this opinion", we get exactly what we want. But research has shown that one species will die out on the long run. In our adopted voter model, an opinion would die out, which is absolutely not the case, thus this very simple approach does not fit our needs very well for the moment. This problem will be addressed in the next subsections.
...
Only the mid part of these dynamics will be clarified in the explanation of the transition rates \eqref{def:transition_rates_zealot}, an exhaustive discussion may be found in \cite{Aguiar2011, Chinellato2015}. \newline
...
\begin{split}
X_t \rightarrow X_t + 1 & \textnormal{ at rate } \mu\left(N-X_t\right)\frac{\theta_X (X_t + N_X + aI)}{\theta_X (X_t + N_X + aI) + \big(N-X_t + N_Y + \Omega\left(I\right) N\big)} \\
&\\
X_t \rightarrow X_t - 1 & \textnormal{ at rate } \mu X_t\frac{\theta_Y\big(N-X_t+N_Y + \Omega\left(I\right) N\big)}{(X_t + N_X+aI) + \theta_Y\big(N-X_t+N_Y + \Omega\left(I\right) N\big)}
\end{split}
\end{align}
\end{model}
...
In addition to the context of Model \ref{model:stochastic_process_my_model}, we will assume that the two zealot groups $N_X$ and $N_Y$ scale linearly with the total population size $N$:
...
\begin{prop}\label{prop:det_limit_my_model_main}
	...
	\begin{split}
	\dot{x} = &-\mu x\frac{\theta_Y\big(1-x+n_Y+\omega\left(i\right)\big)}{(x+n_X+ai) + \theta_Y\big(1-x+n_Y+\omega\left(i\right)\big)}\\
	\qquad&+ \mu \left(1-x\right)\frac{\theta_X (x+ n_X+ ai)}{\theta_X (x + n_X + ai) + \big(1-x + n_Y + \omega\left(i\right)\big)}
	\end{split}
	\end{align}
\end{prop}
...
\begin{proof}
	...
	\begin{align*}
	F_+ \left(X_t\right) &\coloneqq\frac{\theta_X (X_t + N_X + aI)}{\theta_X (X_t + N_X + aI) + \big(N-X_t + N_Y + \Omega\left(I\right) N\big)} \\
	&\\
	F_- \left(X_t\right) &\coloneqq\frac{\theta_Y\big(N-X_t + N_Y + \Omega\left(I\right) N\big)}{(X_t + N_X + aI) + \theta_Y\big(N-X_t + N_Y + \Omega\left(I\right) N\big)}
	\end{align*}
	...
	&\qquad - \big(F_+\left(k\right) + F_-\left(k\right)\big)\times p_k\left(t\right)
	\end{split}
	\end{align}
	...
	\begin{split}
	f_+(x) &\coloneqq \frac{\theta_X (x+ n_X+ ai)}{\theta_X (x + n_X + ai) + \big(1-x + n_Y + \omega\left(i\right)\big)}\\
	f_-(x) &\coloneqq \frac{\theta_Y\big(1-x+n_Y+\omega\left(i\right)\big)}{(x+n_X+ai) + \theta_Y\big(1-x+n_Y+\omega\left(i\right)\big)}
	\end{split}
	\end{align}
	...
	From there we can compute the Taylor series of second order for the $x-h$ and $x+h$ terms:
...
Note that the normal form of the saddle-node bifurcation implies no existing fixed points for $\mu < 0$, exactly one for $\mu = 0$ and two for $\mu > 0$. This is why they are also called ``blue-sky-bifurcations", as they seem to appear ``out of nowhere" \cite{Muller2015}.
...
\begin{prop}[\textbf{A Stationary Point of the Combined Model}]\label{prop:theor_stat_point_my_model}
	Presume $\left(s^*, i^*, x^*\right)$, $i^* \neq 0$ is a stationary point of Model \ref{model:my_model}.	For $n_X = \max \lbrace a\left(1-2i^*\right), 0\rbrace$,\\ $n_Y = a\left(2i^* - 1\right) + n_X$ and $\theta_X = \theta_Y = \frac{1-2n}{1+2n}$ with $n \coloneqq ai^* + n_X$, there always is a stationary point at $x^* = \frac{1}{2}$.
\end{prop} 
\begin{proof}
...
Now, define the function \textit{antivax}$\left(x,i\right): \left[0,1\right]^2 \rightarrow \mathbb{R}$ so that it describes the anti-vaccination impact on $\dot{x}$ and plot it against $x$. To do so, we define \textit{antivax}$\left(x,i\right)$ to be the negative term of the right hand side in \eqref{prop:det_limit_my_model}, when leaving out $\mu$:

\begin{equation*}
\textit{antivax}\left(x,i\right) \coloneqq -x\frac{\theta_Y\big(1-x+n_Y+\omega\left(i\right)\big)}{(x+n_X+ai) + \theta_Y\big(1-x+n_Y+\omega\left(i\right)\big)}
\end{equation*}
...
\begin{proof}
	...
	= & \mu(1-x) \frac{(1-h\Theta_X) (x+ hN_X+ hAi)}{(1-h\Theta_X) (x + hN_X + hAi) + \big(1-x + hN_Y + hA(1-i)\big)}\\
	&\qquad - \mu x \frac{(1-h\Theta_Y)\big(1-x+hN_Y+hA(1-i)\big)}{(x+h_NX+hAi) + (1-h\Theta_Y)\big(1-x+hN_Y+hA(1-i)\big)}\\
	...
	\begin{equation*}
	h\big[\mu(1-x)f_+(x) + \mu xf_-(x)\big] = \mu h 2x(1-x) + \mathcal{O}(h^2).
	\end{equation*}
	...
	\begin{align*}
	0 &= -\Big(\left[\left(\Theta_X + \Theta_Y\right)x - \Theta_X\right]x\left(1-x\right)-\left(N_X + N_Y + A\right)x + Ai + N_X\Big)\phi(x)\\
		&\qquad + \frac{d}{dx}\big(\phi(x)x\left(1-x\right)\big)
	\end{align*}
	
	With $v(x) = \phi(x)x\left(1-x\right)$, we have:
	
	\begin{equation*}
	v'(x) = \left(\big[\left(\Theta_X + \Theta_Y\right)x - \Theta_X\big] + \frac{N_X}{x} - \frac{N_Y}{1-x} + \frac{A(i-x)}{x(1-x)}\right)v(x)
	\end{equation*}
...
\begin{split}\label{eq:my_distribution}
	\phi(x) &= \hat{C}\exp\Bigg[\hat{s}\Bigg\{\hat{\theta}\left(\frac{x^2}{2} - \hat{\psi}x\right)+\ln(x)\left((1-\hat{\theta})\hat{\nu}(1-\hat{\xi})+\hat{\theta}\hat{\xi}i\right)\Bigg.\Bigg.\\
	&\qquad \Bigg.\Bigg.+ \ln(1-x)\left((1-\hat{\theta})(1-\hat{\nu})(1-\hat{\xi})+\hat{\theta}\hat{\xi}(1-i)\right) + B\Bigg\}\Bigg],
\end{split}
\end{align}
...
\ifthenelse{\boolean{appendix}}{\input{appendix_bachelor_thesis_julien_caselmann.tex}}{}
